# ryOS CrossMux: agent skills

Project skills for coding agents. Agents auto-discover them and load one when the
...
This is separate from [`AGENTS.md`](../../AGENTS.md), the always-loaded rule set.
These skills are the applied decision procedures that load on demand and add the
judgment layer `AGENTS.md` does not carry.
...
Each skill ends with a self-review checklist the agent runs against its own diff
...
`AGENTS.md`; add the judgment `AGENTS.md` cannot afford to carry. Trigger quality
