#!/bin/bash
#
# Build lighter EB Garamond + Source Han Serif TC .cpfont files for SD card use.
#
# Uses the same ~7000 通用汉字 Traditional subset as the builtin CN 14pt reader
# font (chars_7000_common.txt → OpenCC s2t).
#
# Layout:
...
# Reader-default symbol ranges (matches build-cn-builtin-fonts.sh LARGE tier).
SUBSET_UNICODES="U+0020-007E,U+00A0-00FF,U+2010-2026,U+2030-205F,U+2070-209F,U+20A0-20CF,U+2150-218F,U+2190-21FF,U+2200-22FF,U+2460-24FF,U+2500-257F,U+2580-259F,U+25A0-25FF,U+2600-26FF,U+2700-27BF,U+3000-303F,U+FE10-FE19,U+FE30-FE48,U+FF00-FFEF,U+FFFD"
LATIN_INTERVALS="latin-ext,symbols,(0x2030-0x205F),(0x2460-0x24FF),(0x2580-0x259F)"
REGULAR_INTERVALS="latin-ext,cjk,symbols,(0x2030-0x205F),(0x2460-0x24FF),(0x2580-0x259F),(0xFE10-0xFE19),(0xFE30-0xFE48)"

for f in "$EB_REGULAR" "$EB_BOLD" "$EB_ITALIC" "$EB_BOLDITALIC" "$SHS_REGULAR" "$LARGE_CHARSET_FILE_SC"; do
  if [ ! -f "$f" ]; then
