...
Pools (official):
  - chars_kyoiku_1026.txt — 学年別漢字配当表 / 教育漢字 (MEXT, 1026 chars)
  - chars_joyo_2136.txt   — 常用漢字表 (内閣告示, 2136 chars; used by 14pt script)
  - chars_ja_kana.txt     — hiragana + katakana

Outputs:
  - ja_common_chars.txt  — 教育漢字 ∪ kana ∪ --require-from (drives 8/10/12pt)
...
KYOIKU_FILE = SCRIPT_DIR / "chars_kyoiku_1026.txt"
...
def main() -> None:
    ...
    beyond = sorted(required - kyoiku - kana)
    print(
        f"Wrote {len(kept)} chars → {out.name} "
        f"(kyoiku {len(kyoiku)} ∪ kana {len(kana)} ∪ required {len(required)})",
        file=sys.stderr,
    )
    print(f"Wrote {len(i18n)} i18n/kana chars → {i18n_out.name}", file=sys.stderr)
    if beyond:
        print(f"[required] +{len(beyond)} beyond kyoiku/kana: {''.join(beyond)}", file=sys.stderr)
