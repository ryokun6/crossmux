...
class TextBlock final : public Block {
 private:
  ...
  // Optional per-word ruby annotations. Empty when the line has no ruby
  // (fork layout does not yet emit ruby; kept for cache format + dictionary).
 ...
 public:
  ...
  // Alias used by dictionary word-select geometry.
  int16_t wordXAt(const uint16_t i) const { return wordXpos(i); }
