...
- 단을 오른쪽에서 왼쪽으로 배치
- CJK 문장 부호에 세로쓰기 표현형 사용. 번체는 CLREQ/대만 관례에 따라 點號(`、。，：；！？`)를 정립·중앙 정렬로 유지하고, 간체는 일본어식 FE 리맵을 따름
- 반복되는 줄임표·대시를 한 칸씩 세로로 연결
- 로마자 구간을 회전하되 짧은 숫자 참조는 읽기 쉽게 유지
- 짧은 가로 조각에 세로 중 가로(tate-chu-yoko) 적용 (단독 영숫자는 전각으로 확장, 두 글자는 반각 유지)
- 리더 설정에서 켜면 번체/간체/일본어에 標點擠壓/約物詰め(구두점 압축) 적용 (기본 켜짐; 한국어·라틴 빌드는 무시)
...
저장소에는 EB Garamond + 지역별 Source Han Serif 빌더(로캘 패밀리 `EBGaramondSHS-{TC,SC,JA,KO}`, 소스는 `lib/EpdFont/scripts/source_fonts/`)도 포함되어 있습니다. 자세한 내용은 [SD-card fonts](./docs/sd-card-fonts.md)를 참고하세요.
...
[https://os.ryo.lu](https://os.ryo.lu)에서 가입한 뒤, 기기에서도 같은 **ryOS 계정**
사용자 이름과 비밀번호를 사용하세요:

- **ryOS 클라우드 동기화** (`설정 → 시스템 → ryOS 클라우드 동기화`) — 여러 기기에서 읽기 진행률을 동기화합니다.
  프로토콜은 KOReader 호환이므로, 같은 서버를 가리키면 KOReader 앱에서도 같은 자격 증명을 쓸 수 있습니다.
- **ryOS 서재** (앱 → ryOS 서재, 또는 `설정 → 시스템 → ryOS 서재`) — 계정에 연결된 서재에서 책을 둘러보고 다운로드합니다.
  기본으로 ryOS 서재 카탈로그가 미리 채워져 있으며, 다른 OPDS 서버(예: 집 Calibre)도 추가할 수 있습니다.
...
- CJK 세로쓰기, 禁則(킨소쿠) 행/단 줄바꿈, 번체/간체/일본어용 標點擠壓/約物詰め
- 자동 페이지 넘김, 화면 방향 제어, 스크린샷, QR 표시
- ryOS 계정으로 하는 ryOS 클라우드 동기화 (KOReader 호환)
