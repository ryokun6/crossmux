...
void EpubReaderMenuActivity::loop() {
  ...
  auto activateSelected = [this] {
    ...
    if (selectedAction == MenuAction::TOGGLE_WRITING_MODE) {
      optionPopup.show(StrId::STR_WRITING_MODE, writingModeLabels.data(), static_cast<int>(writingModeLabels.size()),
                       pendingWritingMode, [this](int idx) {
                         pendingWritingMode = idx;
                         requestUpdate();
                       });
      requestUpdate();
      return;
    ...
    setResult(MenuResult{static_cast<int>(selectedAction), pendingOrientation, selectedPageTurnOption, pendingWritingMode});
    finish();
  };
