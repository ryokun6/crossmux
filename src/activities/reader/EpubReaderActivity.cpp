...
void EpubReaderActivity::openReaderMenu() {
  ...
  startActivityForResult(
      ...
      [this](const ActivityResult& result) {
        ...
        // Always apply orientation/writing-mode change even if the menu was cancelled
        const auto& menu = std::get<MenuResult>(result.data);
        applyOrientation(menu.orientation);
        applyWritingMode(menu.writingMode);
...
void EpubReaderActivity::loop() {
  ...
  if (section && !section->isBuilding() && section->isPartial() && !RenderLock::peek() && buildViewportWidth > 0 &&
    ...
    ReaderRenderSpec buildSpec = SETTINGS.readerRenderSpec(buildViewportWidth, buildViewportHeight);
      buildSpec.writingMode = effectiveWritingMode();
      buildSpec.punctCompressionEnabled = SETTINGS.punctCompressionEnabled != 0;
...
void EpubReaderActivity::onReaderMenuConfirm(EpubReaderMenuActivity::MenuAction action) {
  ...
  switch (action) {
    ...
    case EpubReaderMenuActivity::MenuAction::TOGGLE_WRITING_MODE:
      // Applied via MenuResult.writingMode on menu exit.
      break;
...
void EpubReaderActivity::render(RenderLock&& lock) {
  ...
  renderSpec.writingMode = effectiveWritingMode();
  renderSpec.punctCompressionEnabled = SETTINGS.punctCompressionEnabled != 0;
...
void EpubReaderActivity::renderContents(std::unique_ptr<Page> page, const int orientedMarginTop,
  ...
  if (pageHasImages && page->hasImagesNeedingDecode()) {
    ...
    // through the renderer, so discard those pixels before the real page pass.
