...
const char* difficultyLabel(MinesweeperBoard::Difficulty d) {
  switch (d) {
...
MinesweeperMenuActivity::MinesweeperMenuActivity(GfxRenderer& renderer, MappedInputManager& mappedInput)
    : Activity("MinesweeperMenu", renderer, mappedInput) {}

void MinesweeperMenuActivity::onEnter() {
  Activity::onEnter();
  renderer.setOrientation(GfxRenderer::Orientation::Portrait);
  buildItems();
  selected = 0;
  showingStats = false;
  cachedStats = MinesweeperStore::loadStats();
  requestUpdate();
}

void MinesweeperMenuActivity::onExit() { Activity::onExit(); }

void MinesweeperMenuActivity::buildItems() {
  items.clear();
  items.reserve(5);

  if (MinesweeperStore::hasInProgress()) {
    MinesweeperSaveSlot slot;
    if (MinesweeperStore::load(slot)) {
      resumeElapsedSec = slot.elapsedSec;
      resumeDifficulty = slot.difficulty;
      const uint16_t total = slot.board.totalCells();
      const uint16_t nonMine = (total > slot.board.mineCount) ? (total - slot.board.mineCount) : 0;
      resumeProgressPercent = (nonMine > 0) ? static_cast<uint8_t>((slot.board.revealedCount * 100u) / nonMine) : 0;
      items.push_back({ItemKind::Continue, slot.difficulty});
    }
  }

  items.push_back({ItemKind::NewGame, MinesweeperBoard::Difficulty::Easy});
  items.push_back({ItemKind::NewGame, MinesweeperBoard::Difficulty::Medium});
  items.push_back({ItemKind::NewGame, MinesweeperBoard::Difficulty::Hard});
  items.push_back({ItemKind::Stats, MinesweeperBoard::Difficulty::Easy});
}
...
void MinesweeperMenuActivity::onSelect() {
...
void MinesweeperMenuActivity::render(RenderLock&&) {
  renderer.setOrientation(GfxRenderer::Orientation::Portrait);
...
void MinesweeperMenuActivity::renderList() {
  const auto& metrics = UITheme::getInstance().getMetrics();
  const int sw = renderer.getScreenWidth();
  const int sh = renderer.getScreenHeight();

  GUI.drawHeader(renderer, Rect{0, metrics.topPadding, sw, metrics.headerHeight}, tr(STR_MINESWEEPER_TITLE));
  ...
  auto rowTitle = [this](int i) -> std::string {
    ...
    switch (it.kind) {
      ...
      case ItemKind::NewGame:
        if (it.difficulty == MinesweeperBoard::Difficulty::Medium) return std::string(tr(STR_MINESWEEPER_NEW_MEDIUM));
        if (it.difficulty == MinesweeperBoard::Difficulty::Hard) return std::string(tr(STR_MINESWEEPER_NEW_HARD));
        return std::string(tr(STR_MINESWEEPER_NEW_EASY));
      case ItemKind::Stats:
  ...
  auto rowSubtitle = [this](int i) -> std::string {
    ...
    switch (it.kind) {
      case ItemKind::Continue:
        snprintf(buf, sizeof(buf), "%s · %02u:%02u · %u%%", difficultyLabel(resumeDifficulty),
      ...
      case ItemKind::NewGame:
        if (it.difficulty == MinesweeperBoard::Difficulty::Medium) return std::string(tr(STR_MINESWEEPER_MEDIUM_DESC));
        if (it.difficulty == MinesweeperBoard::Difficulty::Hard) return std::string(tr(STR_MINESWEEPER_HARD_DESC));
        return std::string(tr(STR_MINESWEEPER_EASY_DESC));
      case ItemKind::Stats:
        if (cachedStats.bestTimeSec[0] > 0) {
          snprintf(buf, sizeof(buf), "%s %02u:%02u · %u %s", tr(STR_MINESWEEPER_DIFFICULTY_EASY),
                   static_cast<unsigned>(cachedStats.bestTimeSec[0] / 60),
                   static_cast<unsigned>(cachedStats.bestTimeSec[0] % 60),
                   static_cast<unsigned>(cachedStats.completedCount[0]), tr(STR_MINESWEEPER_COMPLETED));
          return std::string(buf);
        }
...
void MinesweeperMenuActivity::renderStats() {
  const auto& metrics = UITheme::getInstance().getMetrics();
  const int sw = renderer.getScreenWidth();
  const int sh = renderer.getScreenHeight();
  ...
  auto rowTitle = [](int i) -> std::string {
    if (i == 0) return std::string(tr(STR_MINESWEEPER_DIFFICULTY_EASY));
    if (i == 1) return std::string(tr(STR_MINESWEEPER_DIFFICULTY_MEDIUM));
    return std::string(tr(STR_MINESWEEPER_DIFFICULTY_HARD));
  };
  auto rowSubtitle = [this](int i) -> std::string {
    char buf[96];
    if (cachedStats.bestTimeSec[i] > 0) {
      ...
      snprintf(buf, sizeof(buf), "%s %02u:%02u · %u %s · %d%%", tr(STR_GAME_BEST_TIME),
               ...
               static_cast<unsigned>(cachedStats.bestTimeSec[i] % 60),
               static_cast<unsigned>(cachedStats.completedCount[i]), tr(STR_MINESWEEPER_COMPLETED), rate);
    } else {
