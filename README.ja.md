...
- 段組を右から左へ配置
- CJK 約物に縦書き用字形を使用。繁体字は CLREQ／台湾慣例に沿い點號（`、。，：；！？`）を正立・中央揃えのままにし、簡体字は日本語同様の FE 領域リマップを行う
- 連続する三点リーダーやダッシュを 1 マスずつ縦に連結
- 欧文を回転しつつ、短い数字参照は読みやすく維持
- コンパクトな横書き断片に縦中横を適用（単独の英数字は全角へ拡張、2 文字は半角のまま）
- リーダー設定で有効なとき、繁体／簡体／日本語に標點擠壓／約物詰め（句読点圧縮）を適用（既定オン；韓国語・ラテン版は対象外）
...
| サイズ | 収録内容 | 出典 |
| --- | --- | --- |
| 8/10/12pt（最小） | 常用漢字 2,136 字 + かな + UI 文字 | 常用漢字表 (2010) |
| 14pt（既定の「中」／ベース） | JIS X 0208 第1・第2水準 6,355 字 + 常用漢字 + かな + UI 文字 + EPUB 記号 | JIS X 0208 (1990) |
| 16/18pt | かな + UI 文字のみ | — |
...
リポジトリには EB Garamond + 地域別 Source Han Serif のビルダー（ロケールファミリー `EBGaramondSHS-{TC,SC,JA,KO}`、ソースは `lib/EpdFont/scripts/source_fonts/`）も含まれます。詳しくは [SD-card fonts](./docs/sd-card-fonts.md) を参照してください。
...
[https://os.ryo.lu](https://os.ryo.lu) でアカウントを作成し、端末でも同じ **ryOS アカウント**
のユーザー名とパスワードを使います:

- **ryOS クラウド同期**（`設定 → システム → ryOS クラウド同期`）— 複数端末で読書進捗を同期します。
  プロトコルは KOReader 互換なので、同じサーバーを向ければ KOReader アプリでも同じ認証情報を使えます。
- **ryOS 書庫**（アプリ → ryOS 書庫、または `設定 → システム → ryOS 書庫`）— アカウントに紐づく書庫から本を閲覧・ダウンロードします。
  既定で ryOS 書庫カタログが事前入力されています。ほかの OPDS サーバー（例: 自宅の Calibre）も追加できます。
...
- CJK 縦書き（縦組み）、禁則による行／段の折り返し、繁体／簡体／日本語向けの標點擠壓／約物詰め
- 自動ページめくり、画面向き制御、スクリーンショット、QR 表示
- ryOS アカウントによる ryOS クラウド同期（KOReader 互換）
...
- **読書には 14pt（「中」）を推奨** — JIS 第1・第2水準漢字をフル収録するのはこのサイズだけです。8–12pt は常用漢字、16/18pt は UI 文字のみのため、小説の漢字が □ になることがあります。
