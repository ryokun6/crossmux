...
- Slovenščina
- Valencià
- 中文 (Traditional UI strings; language code `ZH`)
- עברית
...
`"ZH"` written by the Chinese build (`ENABLE_CHINESE_VERSION`) is still
*resolvable* by the global build (its strings are compiled in) but renders as
garbled boxes because the global build embeds no CJK font. Legacy `"ZH_CN"`
values in older `settings.json` files are accepted as an alias for `"ZH"`.

Two mechanisms in `JsonSettingsIO::loadSettings()` keep the UI readable:

- **Build SKU marker** — `settings.json` carries a `langSku` field (`"zh"` /
  `"global"`; legacy `"cn"` is treated as `"zh"` on load and rewritten). On load,
  if the stored marker differs from the running build's SKU, the language is
  reset to `CrossPointSettings::defaultLanguageIndex()`
  (EN on the global build, ZH on the Chinese build). A *missing* marker
...
lists Chinese (`中文`; Traditional UI strings).
