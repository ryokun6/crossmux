...
This guide explains the multi-language support system in ryOS CrossMux.
...
- Valencià
<<<<<<< HEAD
- 繁體中文 (Traditional; locale `zh-TW`, enum `ZH_TW`)
- 简体中文 (Simplified; locale `zh-CN`, enum `ZH_CN`; synthesized from Taiwan-terminology Traditional YAML via OpenCC `tw2sp` on SC builds)
- 日本語 (locale `ja-JP`, enum `JA`; `gh_release_ja` SKU)
- 한국어 (locale `ko-KR`, enum `KO`; `gh_release_ko` SKU)
=======
>>>>>>> upstream/master
- עברית
...
### Cross-SKU reflash recovery

`settings.json` lives on the SD card, so it survives a firmware flash. A language
chosen under one SKU can therefore linger into another after a reflash — e.g.
`"zh-TW"` / `"zh-CN"` written by Chinese builds (`ENABLE_CHINESE_VERSION`) may
still be *resolvable* by the global build (ZH_TW strings are compiled into the
full table) but render as garbled boxes because the global build embeds no CJK
font. Legacy `"ZH"`, `"ZH_TW"`, and `"ZH_CN"` values are accepted as aliases.

Two mechanisms in `JsonSettingsIO::loadSettings()` keep the UI readable:

- **Build SKU marker** — `settings.json` carries a `langSku` field (`"zh-tw"` /
  `"zh-cn"` / `"ja"` / `"ko"` / `"global"`; legacy `"zh"` / `"cn"` are treated as
  Traditional on load and rewritten). On load, if the stored marker differs from
  the running build's SKU, the language is reset to
  `CrossPointSettings::defaultLanguageIndex()`
  (EN on global, ZH_TW / ZH_CN / JA / KO on the CJK SKUs). A *missing* marker
  (settings written before this field existed) is treated as the same SKU so a
  deliberate language choice is never clobbered on a normal upgrade.
- **Renderability fallback** — independent of the marker, any language that
  `I18n::isLanguageAvailable()` reports as unrenderable in the current build is
  reset to the default. This rescues existing devices (no marker yet) that were
  flashed Chinese → global.

The same `isLanguageAvailable()` check also hides unrenderable languages from the
in-app language selector (`LanguageSelectActivity`), so the global build never
lists Chinese locales.

`ja` / `ko` are distinct markers rather than sharing `global` because Settings >
Check for updates can now install another language build directly (see
[engineering/build-system.md](engineering/build-system.md)). Between two CJK SKUs
the renderability check does not help — a JA build's generated enum has no `KO`
member, so a leftover `"ko-KR"` falls back to `EN` and a user who asked for the
Korean build would land in an English UI. The marker mismatch resets to that
build's own default instead. The cost is that an existing JA/KO device which had
deliberately selected EN gets reset to JA/KO once, on the first boot after this
change.

New user-facing strings only need to be added to `english.yaml`; other locales
fall back to English automatically. The four CJK YAMLs are worth translating by
hand, because a build's own locale is the one that renders in its embedded font —
and any CJK character added there must already be in that SKU's committed glyph
subset, since the font headers are only regenerated with the (gitignored) source
OTF. Check a new string against
`lib/EpdFont/builtinFonts/notosans_{cjk,sc,ja,ko}_{8,10,12}.h`, whose interval
tables list exactly which codepoints render at UI sizes.
=======
