...
- Slovenščina
- Valencià
- 中文 (Traditional UI strings; language code `ZH_CN`)
- עברית
...
`"ZH_CN"` written by the Chinese build (`ENABLE_CHINESE_VERSION`) is still
*resolvable* by the global build (its strings are compiled in) but renders as
garbled boxes because the global build embeds no CJK font.

Two mechanisms in `JsonSettingsIO::loadSettings()` keep the UI readable:

- **Build SKU marker** — `settings.json` carries a `langSku` field (`"cn"` /
  `"global"`). On load, if the stored marker differs from the running build's
  SKU, the language is reset to `CrossPointSettings::defaultLanguageIndex()`
  (EN on the global build, ZH_CN on the Chinese build). A *missing* marker
