...
- **Build SKU marker** — `settings.json` carries a `langSku` field (`"zh-tw"` /
  `"zh-cn"` / `"global"`; legacy `"zh"` / `"cn"` are treated as Traditional on
  load and rewritten). On load, if the stored marker differs from the running
  build's SKU, the language is reset to `CrossPointSettings::defaultLanguageIndex()`
  (EN on global, ZH_TW / ZH_CN on Chinese SKUs). A *missing* marker
