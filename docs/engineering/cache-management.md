...
- `section.bin`: **per-flavor** — Latin builds **Version 49**, Chinese builds (`ENABLE_CHINESE_VERSION`) **Version 50**. The two flavors emit different word streams (per-character CJK tokenization), so each carries an independent counter; numbers stay distinct so a firmware flavor swap never reuses the other's cache.
