...
> `gh_release_cn` Chinese firmware: how the flag gates resources, the
> flash budget, and how to regenerate the embedded CJK fonts.

A dedicated build env, `gh_release_cn`, produces a Chinese-first
firmware with Traditional UI strings. The `-DENABLE_CHINESE_VERSION` flag in [platformio.ini](../../platformio.ini)
