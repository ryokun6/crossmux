...
| `gh_release_sc` | `zh-CN` | Traditional YAML → OpenCC **tw2sp** (TW terms → Mainland Simplified) at gen_i18n | Source Han Sans CN → `notosans_sc_*.h` (same `notosans_cjk_*` symbols) | TC→SC (`TcToScRemap.h`) | `firmware-sc.bin` | `-sc` |
