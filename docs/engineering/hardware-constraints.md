...
10. Drop the SD reader font before Wi‑Fi/TLS: a loaded SD `.cpfont` pins roughly 75KB of the ~224KB internal heap (kern-class tables, ligature pairs, interval tables, per-style mini glyph bitmaps) for as long as it is registered. That single allocation is the difference between an X3 serving HTTP at `MaxAlloc~2KB` — below lwIP's ~5.7KB TCP send buffer — and one at `MaxAlloc~57KB`, and it puts free heap under `KOReaderSyncClient`'s 40KB TLS floor outright. Any activity that brings up Wi‑Fi or opens a TLS connection must hold a `ScopedSdFontUnload` (`src/ScopedSdFontUnload.h`) for the duration, or call `sdFontSystem.unloadAll(renderer)` / `ensureLoaded(renderer)` by hand as the OTA and font-download screens still do. Before adding one, check that the screen does not render book text with the reader font — unloading is invisible on UI-font screens, but it would silently swap the reader's typeface. Paths that only speak UDP (NTP/SNTP) do not need it.
